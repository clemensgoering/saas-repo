...
import ThemeToggler from "@repo/system/ThemeToggler";
      ...
      <div className="relative mx-auto max-w-c-1390 items-center justify-between px-4 md:px-8 xl:flex 2xl:px-0">
          ...
          >
            <span className="relative block h-5.5 w-5.5 cursor-pointer">
              <span className="absolute right-0 block h-full w-full">
                <span
                  className={`relative left-0 top-0 my-1 block h-0.5 rounded-sm bg-black duration-200 ease-in-out dark:bg-white ${!navigationOpen ? "!w-full delay-300" : "w-0"
                    }`}
                ></span>
                <span
              ...
              <span className="du-block absolute right-0 h-full w-full rotate-45">
                <span
                  className={`absolute left-2.5 top-0 block h-full w-0.5 rounded-sm bg-black delay-300 duration-200 ease-in-out dark:bg-white ${!navigationOpen ? "!h-0" : "h-full"
                    }`}
                ></span>
                <span
        ...
        >
                      ...
                      <Link
                        ...
                        className={
                          pathUrl === menuItem.path
                            ...
                            : "hover:text-blacksection hover:font-medium dark:hover:text-white"
                        }
                      >
                        {menuItem.title}
                      </Link>
          ...
          <div className="mt-7 flex items-center gap-6 xl:mt-0">
            {children}
            {config?.themeToggler !== undefined && config?.themeToggler === true ? (
              <ThemeToggler />
            ) : ""}
          </div>
        </div>
